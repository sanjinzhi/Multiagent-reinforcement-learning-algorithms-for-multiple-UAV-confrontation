    ...
    def step(self, action_n):
        ...
        for i, agent in enumerate(self.agents):
            if agent.death:
                state_dim = 0
                #if agent.adversary:
                    #state_dim = 4+2*(self.n-1)+2*(self.n-1-green_agent_num)
                #else:
                    #state_dim = 4+2*(self.n-1)+2*(self.n-1-red_agent_num)
                state_dim = 4+4*(self.n-1)+self.n
        ...
        for i, agent in enumerate(self.agents):
            if agent.death and i not in first_death:
                #state_dim = 4+4*(self.n-1)+self.n
